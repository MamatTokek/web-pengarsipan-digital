...
@section('content')
<h1 class="text-3xl font-semibold text-gray-800">Kelola Surat > Lihat Detail</h1>

{{-- Container utama dengan x-data untuk kontrol modal - Menambahkan variabel qrUrl untuk menampung link QR API --}}
<div class="container mx-auto px-4 py-8" x-data="{ openModal: false, previewUrl: '', previewTitle: '', qrUrl: '' }">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-2 gap-8 items-stretch">
        ...
        {{-- SISI KIRI: DETAIL SURAT MASUK (REFERENSI) --}}
        <div class="flex">
            <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden flex flex-col w-full">
                ...
                <div class="p-6 flex-grow">
                    <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                        ...
                        {{-- MODIFIKASI: Menambahkan Nomor Surat --}}
                        <div class="sm:col-span-2">
                            <dt class="text-xs font-bold text-gray-500 uppercase tracking-widest">Nomor Surat</dt>
                            <dd class="mt-1 text-sm font-mono text-indigo-600 font-bold">{{ $letter->letter_number ?? 'Belum Diatur' }}</dd>
                        </div>
                        ...
                        <div>
                            <dt class="text-xs font-bold text-gray-500 uppercase tracking-widest">Tanggal Upload</dt>
                            <dd class="mt-1 text-sm text-gray-900">{{ $letter->uploaded_at->format('d F Y') }}</dd>
                        </div>
                        <div class="sm:col-span-2">
                ...
                <div class="p-6 border-t border-gray-100 space-y-3">
                    {{-- TOMBOL BERSEBELAHAN (LIHAT & EDIT) --}}
                    <div class="grid grid-cols-2 gap-4">
                        ...
                        <a href="{{ route('letters.edit', [$letter->id, 'source' => 'detail']) }}" 
                           class="flex items-center justify-center bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-3 px-4 rounded-lg shadow-md transition duration-150 transform hover:-translate-y-0.5 uppercase tracking-wider text-sm">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    {{-- TOMBOL DOWNLOAD SURAT ASLI --}}
                    <a href="{{ route('letters.download', $letter->id) }}" class="flex items-center justify-center w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-lg shadow-md transition duration-150 transform hover:-translate-y-0.5 uppercase tracking-wider text-sm">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div class="flex">
            <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden flex flex-col w-full">
                <div class="{{ $letter->replyLetter ? 'bg-green-600' : 'bg-orange-500' }} px-6 py-4">
                    <h3 class="text-white font-bold flex items-center uppercase tracking-wider text-sm">
                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" />
                        </svg>
                        {{ $letter->replyLetter ? 'Dokumen Balasan (Selesai)' : 'Status: Perlu Tindakan' }}
                    </h3>
                </div>
                    ...
                    @if($letter->replyLetter)
                        <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                            <div class="sm:col-span-2">
                                ...
                                <dd class="mt-1 text-sm font-semibold text-gray-900">{{ $letter->replyLetter->name }}</dd>
                            </div>

                            {{-- Menampilkan Nomor Surat pada Dokumen Balasan jika tersedia --}}
                            @if($letter->replyLetter->letter_number)
                            <div class="sm:col-span-2">
                                <dt class="text-xs font-bold text-gray-500 uppercase tracking-widest">Nomor Surat Balasan</dt>
                                <dd class="mt-1 text-sm font-mono text-green-600 font-bold">{{ $letter->replyLetter->letter_number }}</dd>
                            </div>
                            @endif

                            <div>
                                <dt class="text-xs font-bold text-gray-500 uppercase tracking-widest">Tanggal Balasan</dt>
                                <dd class="mt-1 text-sm text-gray-900">{{ $letter->replyLetter->uploaded_at->format('d F Y') }}</dd>
                            </div>
                            <div>
                            ...
                            <div class="sm:col-span-2">
                                ...
                                <dd class="mt-1 p-3 bg-indigo-50 border border-indigo-100 rounded-md text-sm text-indigo-700 flex items-center">
                                    ...
                                    {{ $letter->replyLetter->original_file_name }}
                                </dd>
                            </div>
                        </dl>
                    @else
                        <div class="h-full flex flex-col items-center justify-center text-center py-10">
                            ...
                            <h4 class="text-lg font-bold text-gray-800">Menunggu Balasan Kades</h4>
                            <p class="text-gray-500 text-sm mt-2 max-w-xs">Dokumen ini belum memiliki balasan dari Kepala Desa.</p>
                        </div>
                    @endif
                </div>
                ...
                <div class="p-6 border-t border-gray-100">
                    @if($letter->replyLetter)
                        <div class="grid grid-cols-2 gap-4">
                            {{-- LIHAT BALASAN --}}
                            <button type="button" 
                                @click="previewUrl = '{{ route('letters.show', $letter->replyLetter->id) }}?t={{ time() }}'; 
                                        ...
                                        openModal = true"
                                class="flex items-center justify-center w-full bg-green-600 hover:bg-green-700 text-white font-bold py-3 px-4 rounded-lg shadow-md transition duration-150 transform hover:-translate-y-0.5 uppercase tracking-wider text-sm">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" />
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z" />
                                </svg>
                            ...
                            {{-- UNDUH BALASAN --}}
                            <a href="{{ route('letters.download', $letter->replyLetter->id) }}" class="flex items-center justify-center w-full bg-emerald-600 hover:bg-emerald-700 text-white font-bold py-3 px-4 rounded-lg shadow-md transition duration-150 transform hover:-translate-y-0.5 uppercase tracking-wider text-sm">
                                <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a2 2 0 002 2h12a2 2 0 002-2v-1m-4-4l-4 4m0 0l-4-4m4 4V4" />
                                </svg>
                                Unduh Balasan
                    ...
                    @else
                        <div class="flex items-center justify-center w-full bg-gray-100 text-gray-400 font-bold py-3 px-4 rounded-lg border border-dashed border-gray-300 uppercase tracking-wider text-sm cursor-not-allowed">
                            Belum Ada Balasan
                        </div>
                    @endif
                </div>
    ...
    <div class="mt-10 flex justify-center">
        <a href="{{ route('letters.index') }}" class="group flex items-center bg-white border-2 border-gray-200 text-gray-600 hover:text-gray-800 hover:border-gray-400 font-bold py-3 px-10 rounded-xl shadow-sm transition-all duration-200 uppercase text-xs tracking-[0.2em]">
    ...
    {{-- MODAL PREVIEW DENGAN QR CODE (Disamakan dengan index.blade.php) --}}
