...
@section('content')
<h1 class="text-3xl font-semibold text-gray-800">Daftar Dokumen > Edit Balasan</h1>

{{-- MODIFIKASI: Menambahkan variabel untuk kontrol modal sukses dan error salin nomor --}}
<div class="container mx-auto px-4 py-8" x-data="{ 
            ...
            <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden flex flex-col w-full">
                ...
                <div class="p-6 flex-grow">
                    <dl class="grid grid-cols-1 gap-x-4 gap-y-6 sm:grid-cols-2">
                        ...
                        <div class="sm:col-span-2">
                            <dt class="text-xs font-bold text-gray-500 uppercase tracking-widest">Nomor Surat Asli</dt>
                ...
                <div class="p-6 border-t border-gray-100">
                    <button type="button" 
                        @click="previewUrl = '{{ route('letters.show', $letter->id) }}'; 
                                ...
                                openModal = true"
                        class="flex items-center justify-center w-full bg-gray-700 hover:bg-gray-800 text-white font-bold py-3 px-4 rounded-lg shadow-md transition duration-150 transform hover:-translate-y-0.5 uppercase tracking-wider text-sm">
            ...
            <div class="bg-white rounded-xl shadow-lg border border-gray-100 overflow-hidden flex flex-col w-full">
                <div class="bg-indigo-600 px-6 py-4">
                    <h3 class="text-white font-bold flex items-center uppercase tracking-wider text-sm">
                        ...
                        Formulir Edit Balasan
                    </h3>
                </div>
                
                <form action="{{ route('kades.documents.update_reply', $letter->id) }}" method="POST" enctype="multipart/form-data" class="flex flex-col flex-grow">
                    @csrf
                    @method('PUT')
                    <input type="hidden" name="source" value="{{ request('source') }}">

                    <div class="p-6 space-y-4 flex-grow">
                        {{-- 1. NOMOR SURAT BALASAN (FINAL) --}}
                        <div>
                            <label for="letter_number" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Nomor Surat Balasan</label>
                            <input type="text" name="letter_number" id="letter_number" required readonly
                                   class="w-full px-4 py-3 rounded-lg border border-gray-300 bg-gray-50 font-mono font-bold text-indigo-700 @error('letter_number') border-red-500 @enderror"
                                   value="{{ old('letter_number', $reply->letter_number) }}">
                            @error('letter_number')
                                ...
                                <div>
                                    ...
                                    <select id="auto_kode_select" class="w-full p-2 border border-gray-300 rounded-md text-sm shadow-sm focus:ring-indigo-500">
                                        ...
                                        @foreach($categories as $cat)
                                            <option value="{{ $cat->kode_surat }}" 
                                                    {{-- Logika agar kategori yang sudah ada otomatis terpilih --}}
                                                    @if(isset($reply->letter_number) && strpos($reply->letter_number, $cat->kode_surat) === 0) selected @endif>
                                                {{ $cat->kode_surat }} ({{ $cat->name }})
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                        ...
                        <div>
                            <label for="file" class="block text-xs font-bold text-gray-700 uppercase tracking-wider mb-2">Ganti File Balasan (Opsional)</label>
                            ...
                            <div class="mt-3 p-3 bg-gray-50 rounded-lg border border-gray-200">
                                <p class="text-[11px] text-gray-500 flex items-center">
                                    <svg class="w-4 h-4 mr-1 text-indigo-500" fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M4 4a2 2 0 012-2h4.586A1 1 0 0111 2.414l2.586 2.586a1 1 0 01.293.707V16a2 2 0 01-2 2H6a2 2 0 01-2-2V4z" />
                                    </svg>
                                    File saat ini: <span class="font-medium text-indigo-700">{{ $reply->original_file_name }}</span>
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="p-6 border-t border-gray-100 grid grid-cols-2 gap-4">
                        <a href="{{ request('source') === 'detail' ? route('kades.documents.show_action', $letter->id) : route('kades.documents.index') }}" 
                            class="flex items-center justify-center bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold py-3 px-6 rounded-lg transition text-center transform hover:-translate-y-0.5 uppercase text-sm">
                            Batal
                        </a>
    ...
    {{-- MODAL NOTIFIKASI SALIN NOMOR (SUKSES) --}}
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function parseExistingNumber() {
        const existingNumber = finalInput.value;
    ...
    // Jalankan parsing saat halaman siap
    parseExistingNumber();

    // Event Listener untuk perubahan Kode Jenis (Otomatis ambil No Urut)
    ...
    // Update rangkaian nomor setiap ada input manual
    ...
    replyInput.addEventListener('change', function() {
        ...
        if (file) {
            ...
            if (ext === 'pdf') {
                ...
                // Field tidak dikosongkan secara paksa agar seragam dengan Tambah Surat
            }
        }
    });
});
