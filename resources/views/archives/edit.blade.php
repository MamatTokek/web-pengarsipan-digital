...
<div x-data="{ 
    ...
    isSaving: false, 
    usePerakit: false,
    successMessage: '{{ session('success') ?? '' }}', // Gabungkan session Laravel ke dalam variabel Alpine
    togglePerakit() {
        ...
        if (this.usePerakit) {
            ...
            finalInput.value = '{{ $archive->letter_number }}'; // Kembali ke nilai awal saat tidak pakai perakit
        }
    }
}">

    <h1 class="text-3xl font-semibold text-gray-800 mb-6">Kelola Arsip > Edit Arsip</h1>

    {{-- NOTIFIKASI SUKSES (Tampil jika successMessage berisi teks, baik dari Session maupun AJAX) --}}
    ...
    <div class="bg-white p-6 rounded-lg shadow-md max-w-lg mx-auto">
        ...
        <form action="{{ route('archives.update', $archive->id) }}" method="POST" enctype="multipart/form-data" id="archiveForm">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <input type="text" name="letter_number" id="letter_number" readonly
                       ...
                       value="{{ old('letter_number', $archive->letter_number) }}" placeholder="Gunakan perakit di bawah untuk mengisi...">
                @error('letter_number')
            ...
            <div class="mb-4">
                ...
                <textarea name="description" id="description" rows="3" required
                    class="mt-1 block w-full border border-gray-300 rounded-md shadow-sm p-2 focus:ring-indigo-500 focus:border-indigo-500 @error('description') border-red-500 @enderror">{{ old('description', $archive->description) }}</textarea>
            </div>
        ...
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
            ...
            <div class="inline-block align-bottom bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg sm:w-full">
                ...
                <div class="bg-gray-50 px-4 py-3 sm:px-6 sm:flex sm:flex-row-reverse gap-3">
                    <button type="button" @click="saveCategory()" :disabled="isSaving" class="w-full inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-indigo-600 text-base font-bold text-white hover:bg-indigo-700 focus:outline-none sm:ml-3 sm:w-auto sm:text-sm disabled:opacity-50">
...
<script>
// Fungsi rakitNomor global
function rakitNomor() {
    const alpineEl = document.querySelector('[x-data]');
    let alpineData = alpineEl.__x ? alpineEl.__x.$data : Alpine.$data(alpineEl);
